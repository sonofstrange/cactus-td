...
def draw_mechanics_guide_modal(surface, mouse_pos, current_tab=0, context="combat"):
    ...
    if context == "tree":
        ...
        sub_info_str = "Таланты • Назначение веток • Тёмный Космос • 100% Сброс"
        tabs = [
            ("ОСНОВНЫЕ ВЕТКИ", (255, 190, 60)),
            ("ТЁМНЫЙ КОСМОС", (190, 110, 255)),
            ("ВАЛЮТЫ И СОВЕТЫ", (80, 220, 140))
        ]
    elif context == "greenhouse":
        accent_col = (80, 220, 140)
        title_str = "СПРАВОЧНИК: ОРАНЖЕРЕЯ КАКТУСОВ"
        sub_info_str = "Пассивные бонусы • Полив и ростки • Группы сортов • Синергии"
        tabs = [
            ("КАК РАБОТАЕТ ОРАНЖЕРЕЯ", (80, 225, 130)),
            ("ГРУППЫ СОРТОВ И РОСТ", (255, 210, 70)),
            ("СИНЕРГИИ И ПРЕСТИЖ", (100, 200, 255))
        ]
    elif context == "relics":
        accent_col = (255, 180, 75)
        title_str = "СПРАВОЧНИК: МУЗЕЙ И РЕЛИКВИИ"
        sub_info_str = "Археология • Пьедесталы • Коллекция • Советы по сборкам"
        tabs = [
            ("КАК РАБОТАЕТ МУЗЕЙ", (255, 195, 75)),
            ("РАСКОПКИ И АРТЕФАКТЫ", (185, 120, 255)),
            ("СОВЕТЫ ПО СБОРКАМ", (90, 215, 255))
        ]
    else:
        accent_col = (65, 170, 240)
        title_str = "СПРАВОЧНИК: БОЙ, БАШНИ И ТАКТИКА"
        sub_info_str = "Связки башен • Типы урона • Экономика • Советы по тактике"
        tabs = [
            ("ЛУЧШИЕ СВЯЗКИ И КОМБО", (255, 160, 50)),
            ("ТИПЫ УРОНА И БРОНЯ", (80, 220, 130)),
            ("ЭКОНОМИКА И ТАКТИКА", (255, 215, 60))
        ]
    ...
    if context == "tree":
        if current_tab == 0:
            draw_guide_card(
                "ВЕТКА «СИЛА» (БОЕВОЙ ПОТЕНЦИАЛ)",
                (255, 140, 40),
                [
                    ("• За что отвечает: ", "Повышает общий урон всех башен, шанс критического удара и скорость атаки.", (240, 245, 255)),
                    ("• Когда качать: ", "Если слаймы начинают прорываться к базе или не хватает урона по боссам.", (255, 215, 80)),
                    ("• Совет: ", "Критический урон лучше всего раскрывается на скорострельных башнях и снайперах.", (140, 245, 170))
                ],
                badge_text="УРОН И ДПС",
                icons=[damage_upg_icon, speed_upg_icon]
            )
            draw_guide_card(
                "ВЕТКА «ЭКОНОМИКА» (ЗОЛОТО И СТАРТ)",
                (255, 210, 60),
                [
                    ("• За что отвечает: ", "Увеличивает доход за убийство слаймов, стартовый капитал и открывает Фермы.", (240, 245, 255)),
                    ("• Когда качать: ", "Основа для легкого старта любого забега и быстрого разгона золота.", (140, 255, 180)),
                    ("• Совет: ", "Стартовые кактусы позволяют поставить ферму или сильные башни с первой же волны.", (255, 220, 120))
                ],
                badge_text="ЭКОНОМИКА",
                icons=[bounty_upg_icon, farm_tower_img]
            )
            draw_guide_card(
                "ВЕТКА «ВООРУЖЕНИЕ» (ПОТОЛОК СИЛЫ)",
                (80, 210, 255),
                [
                    ("• За что отвечает: ", "Поднимает макс. уровень башен с 20 до 30, даёт стартовые ранги и автосбор.", (240, 245, 255)),
                    ("• Когда качать: ", "Необходима для рубежей 50+ волн, где башни 20 уровня уже не справляются.", (140, 245, 170)),
                    ("• Совет: ", "Стартовые уровни башен экономят огромное количество золота в разгар битвы.", (255, 215, 90))
                ],
                badge_text="РАНГ И ЛИМИТ",
                icons=[crown_upg_icon, start_lvl_icon]
            )
        elif current_tab == 1:
            draw_guide_card(
                "МЕТЕОРИТЫ И ТЁМНЫЙ РЕСУРС",
                (195, 120, 255),
                [
                    ("• Как работает: ", "Астральный Маяк вызывает падение метеоритов на дорогу каждые 15–20 волн.", (255, 215, 80)),
                    ("• Награда: ", "Разбитый метеорит даёт редкие Тёмные кактусы для эндгейм-прокачки.", (220, 160, 255)),
                    ("• Совет: ", "Квантовый Жатватель увеличивает сбор тёмных кактусов с каждого метеорита.", (140, 245, 170))
                ],
                badge_text="РЕСУРСЫ",
                icons=[dark_cactus_img, meteorite_img]
            )
            draw_guide_card(
                "ТЁМНЫЙ ЭГИС И ВОРОНКА [F]",
                (120, 220, 255),
                [
                    ("• Защита базы: ", "Эгис даёт щит базе, поглощая прорывы врагов без потери жизней.", (120, 240, 255)),
                    ("• Орбитальный Залп: ", "Таланты расширяют радиус и урон воронки [F], затягивающей толпы мобов.", (255, 180, 140)),
                    ("• Совет: ", "Эгис спасает от случайных шустрых слаймов, проскочивших основную оборону.", (140, 255, 180))
                ],
                badge_text="ОБОРОНА",
                icons=[crown_upg_icon, dark_cactus_img]
            )
            draw_guide_card(
                "КОРОНА ОАЗИСА (35 УРОВЕНЬ)",
                (255, 215, 75),
                [
                    ("• Сверхпрокачка: ", "Снимает лимит и позволяет улучшать башни в бою вплоть до 35 уровня.", (255, 220, 100)),
                    ("• Зачем нужно: ", "Открывает колоссальный урон для бесконечных режимов и тяжелых карт.", (235, 175, 255)),
                    ("• Совет: ", "Открывайте в эндгейме, когда базовые ветки древа уже достаточно прокачаны.", (140, 255, 180))
                ],
                badge_text="ЭНДГЕЙМ",
            ...
            draw_guide_card(
                ...
                (255, 205, 80),
                [
                    ("• Звёздные кактусы: ", "Выпадают со слаймов, гарантированно с боссов (каждые 5 волн) и в раскопках.", (255, 235, 140)),
                    ("• Тёмные кактусы: ", "Добываются только из разбитых метеоритов и на супер-волнах.", (220, 140, 255)),
                    ("• Рубежи волн: ", "Прохождение 10, 25, 50, 75, 100 волн даёт крупные разовые награды.", (140, 245, 170))
                ],
                badge_text="ДОБЫЧА",
                icons=[stellar_cactus_img_m, dark_cactus_img_m]
            )
            draw_guide_card(
                "БЫСТРЫЙ СТАРТ С ВЫСОКИХ ВОЛН",
                (100, 230, 255),
                [
                    ("• Как работает: ", "Пройденные рубежи открывают возможность начинать забег сразу с высоких волн.", (240, 245, 255)),
                    ("• Экономия времени: ", "Вы пропускаете легкие начальные волны и сразу получаете стартовое золото.", (140, 255, 180)),
                    ("• Совет: ", "Идеальный способ быстро фармить звёзды на уже пройденных картах.", (255, 215, 90))
                ],
                badge_text="ПРОПУСК ВОЛН",
                icons=[start_lvl_icon, trophy_icon]
            )
            draw_guide_card(
                "БЕСПЛАТНЫЙ СБРОС (100% ВОЗВРАТ)",
                (80, 240, 150),
                [
                    ("• Без штрафов: ", "Сброс древа в Настройках абсолютно бесплатен и доступен в любой момент.", (240, 245, 255)),
                    ("• 100% возврат: ", "Все потраченные Звёздные и Тёмные кактусы возвращаются в полном объёме.", (120, 255, 160)),
                    ("• Главный совет: ", "Не бойтесь экспериментировать и перекачиваться под конкретную карту или тактику!", (255, 220, 110))
                ],
                badge_text="СБРОС 100%",
    ...
    # -------------------------------------------------------------
        ...
        if current_tab == 0:
            draw_guide_card(
                "СУТЬ МЕХАНИКИ: ПАССИВНЫЙ РОСТ",
                (80, 225, 130),
                [
                    ("• Глобальные бонусы: ", "Все выращенные кактусы дают постоянные бонусы во всех боях на всех картах.", (240, 245, 255)),
                    ("• Рост со временем: ", "Растения созревают сами в фоновом режиме, пока вы проходите волны в бою.", (140, 255, 180)),
                    ("• Совет: ", "Заглядывайте в оранжерею между боями, чтобы собирать урожай и растить уровни.", (255, 220, 100))
                ],
                badge_text="ПАССИВКА",
            ...
            draw_guide_card(
                ...
                (80, 210, 255),
                [
                    ("• Ускорение вдвое: ", "Полив лейкой удваивает скорость созревания кактусов на 2 минуты.", (120, 245, 160)),
                    ("• Запас воды: ", "Построенные фермы в бою автоматически поставляют росу для вашей лейки.", (240, 245, 255)),
                    ("• Совет: ", "Поливайте оранжерею перед серией волн, чтобы саженцы копились в 2 раза быстрее.", (255, 215, 90))
                ],
                badge_text="ПОЛИВ Х2",
            ...
            draw_guide_card(
                ...
                (140, 240, 160),
                [
                    ("• Сбор урожая: ", "Созревшие кактусы дают саженцы, нужные для перехода на следующий уровень.", (240, 245, 255)),
                    ("• Буфер накопления: ", "Каждый горшок может удерживать до 10 готовых сборов без потерь.", (140, 255, 180)),
                    ("• Совет: ", "Первым делом откройте все слоты горшков — разнообразие сортов выгоднее.", (255, 215, 90))
                ],
                badge_text="УРОЖАЙ",
                icons=[sprout_icon]
            )
            ...
            draw_guide_card(
                "СОРТА ЗАЩИТЫ И ЭКОНОМИКИ",
                (80, 225, 130),
                [
                    ("• Сагуаро (База): ", "Увеличивает здоровье базы, силу шипов и даёт шанс заблокировать урон.", (240, 245, 255)),
                    ("• Опунция (Золото): ", "Даёт солидную прибавку к стартовому золоту в каждом бою и ускоряет фермы.", (255, 220, 100)),
                    ("• Мамиллярия (Воины): ", "Ускоряет возрождение солдат палатки, повышает их HP и даёт щиты.", (140, 255, 180))
                ],
                badge_text="БАЗА И ДОХОД",
                icons=[gh_tex.get("gh_saguaro", cactus_img), gh_tex.get("gh_opuntia", cactus_img)]
            )
            draw_guide_card(
                "СТИХИЙНЫЕ СОРТА (ОГОНЬ И ЛЁД)",
                (255, 140, 80),
                [
                    ("• Огненный сорт: ", "Увеличивает урон огня, продлевает горение и вешает уязвимость на врагов.", (255, 140, 100)),
                    ("• Ледяной сорт: ", "Усиливает замедление холода, радиус башен и срезает броню слаймов.", (100, 220, 255)),
                    ("• Синергия: ", "Совместная прокачка огня и льда превращает комбо «Термошок» в грозное оружие.", (255, 215, 80))
                ],
                badge_text="ОГОНЬ И ЛЁД",
                icons=[gh_tex.get("gh_fire_barrel", cactus_img), gh_tex.get("gh_frost_aloe", cactus_img)]
            )
            draw_guide_card(
                "СОРТА МОЛНИИ, БЕЗДНЫ И ДРОНА",
                (195, 130, 255),
                [
                    ("• Громовой сорт: ", "Добавляет рикошеты цепи молний башни Тесла и даёт шанс оглушения.", (255, 240, 120)),
                    ("• Сорт Бездны: ", "Увеличивает урон по боссам и Теневым врагам, пробивая их защиту.", (210, 140, 255)),
                    ("• Звёздный сорт: ", "Разгоняет урон орбитального Дрона и позволяет ему атаковать 2 цели сразу.", (130, 245, 170))
                ],
                badge_text="ТЕХНОЛОГИИ",
            ...
            draw_guide_card(
                ...
                (255, 195, 75),
                [
                    ("• Как работает: ", "Усиливает силу ВСЕХ бонусов оранжереи разом на +10% за каждый уровень таланта.", (240, 245, 255)),
                    ("• Масштаб: ", "Одновременно повышает здоровье базы, урон стихий, стартовое золото и молнии.", (140, 255, 180)),
                    ("• Совет: ", "Качайте в Древе талантов, когда у вас уже подросли несколько сортов кактусов.", (255, 215, 90))
                ],
                badge_text="+10%/УР. ВСЕМ",
            ...
            draw_guide_card(
                ...
                (100, 230, 255),
                [
                    ("• Суть таланта: ", "Конвертирует суммарные уровни сортов в постоянную прибавку к урону ВСЕХ башен.", (255, 220, 100)),
                    ("• Общий вклад: ", "Каждый прокачанный уровень в любом горшке делает всю армию башен сильнее.", (140, 255, 180)),
                    ("• Совет: ", "Не забрасывайте дешевые кактусы — их уровни тоже идут в копилку общего урона.", (240, 245, 255))
                ],
                badge_text="УРОН БАШНЯМ",
            ...
            draw_guide_card(
                ...
                (80, 240, 150),
                [
                    ("• Что такое: ", "Полная прокачка всех 8 сортов позволяет запустить селекционный Престиж.", (255, 215, 90)),
                    ("• Награда: ", "Даёт мощный глобальный множитель x1.25 ко всем эффектам флоры навсегда.", (120, 255, 160)),
                    ("• Безопасность: ", "Все Звёздные и Тёмные кактусы при престиже сохраняются в полном объёме.", (240, 245, 255))
                ],
                badge_text="ПРЕСТИЖ Х1.25",
    ...
    # -------------------------------------------------------------
        ...
        if current_tab == 0:
            draw_guide_card(
                "СУТЬ МЕХАНИКИ: АКТИВНЫЕ ПЬЕДЕСТАЛЫ",
                (255, 210, 60),
                [
                    ("• Как работает: ", "В музее выставляются найденные артефакты. Пьедесталы дают 100% их силы в бой.", (240, 245, 255)),
                    ("• Слоты: ", "Базово доступно 2 пьедестала, а таланты Древа расширяют их число до 5 слотов.", (140, 255, 180)),
                    ("• Управление: ", "Клик по реликвии выставляет её на пьедестал, клик по пьедесталу снимает.", (255, 215, 90))
                ],
                badge_text="100% ЭФФЕКТ",
                icons=[relic_icon, trophy_icon]
            )
            draw_guide_card(
                "ТАЛАНТ «ТЁМНЫЙ РЕЗОНАНС»",
                (195, 120, 255),
                [
                    ("• Главная пассивка: ", "Позволяет ВСЕЙ коллекции музея работать в фоне даже без пьедесталов!", (220, 160, 255)),
                    ("• Сила коллекции: ", "Каждая открытая и прокачанная реликвия усиливает вашу базу навсегда.", (140, 255, 180)),
                    ("• Совет: ", "Обязательно откройте этот талант в Древе, чтобы реликвии не пылились на полках.", (255, 220, 100))
                ],
                badge_text="ПАССИВКА",
            ...
            draw_guide_card(
                ...
                (255, 215, 75),
                [
                    ("• Уровни артефактов: ", "Повторные находки фрагментов повышают уровень реликвии и её силу.", (240, 245, 255)),
                    ("• Архивы Музея: ", "Талант в Древе расширяет максимальный кап прокачки артефактов с 5 до 20 уровня.", (255, 215, 90)),
                    ("• Рост силы: ", "Бонус реликвии растёт пропорционально её уровню без затухания.", (140, 255, 180))
                ],
                badge_text="КАП 5..20 УР.",
                icons=[crown_upg_icon, relic_icon]
            )
        elif current_tab == 1:
            draw_guide_card(
                "МИНИ-ИГРА РАСКОПОК 5х5 [R]",
                (255, 195, 75),
                [
                    ("• Появление: ", "Во время волны на карте возникает древний курган. Клавиша [R] открывает раскопки.", (240, 245, 255)),
                    ("• Правила: ", "В сетке 5х5 скрыта фигура реликвии (1х2, 1х3, 2х2). Цифры подсказывают расстояние.", (255, 215, 90)),
                    ("• Совет: ", "Талант «Чутьё» даёт больше ходов, гарантируя успешное извлечение фигуры.", (140, 255, 180))
                ],
                badge_text="РАСКОПКИ [R]",
                icons=[shovel_icon]
            )
            draw_guide_card(
                "20 РЕЛИКВИЙ В 10 БИОМАХ",
                (100, 220, 255),
                [
                    ("• Разделение: ", "Каждая карта скрывает по 2 уникальные реликвии под свою тематику.", (240, 245, 255)),
                    ("• Ранние биомы: ", "Артефакты на доход ферм, дальность башен и скидку на первую постройку.", (140, 255, 180)),
                    ("• Поздние биомы: ", "Артефакты на цепную молнию, срез брони врагов и шанс звёздных кактусов.", (220, 160, 255))
                ],
                badge_text="20 АРТЕФАКТОВ",
                icons=[relic_icon, tesla_tower_img]
            )
            draw_guide_card(
                "ЗАМАКШЕННЫЕ БИОМЫ ДАЮТ ЗВЁЗДЫ",
                (80, 240, 150),
                [
                    ("• Чистый фарм: ", "Когда обе реликвии карты полностью прокачаны, раскопки дают чистые Звёзды.", (255, 220, 100)),
                    ("• Безопасность: ", "Раскопки ставят игру на паузу — копайте спокойно прямо посреди боя.", (240, 245, 255)),
                    ("• Совет: ", "Возвращайтесь на освоенные карты для быстрой добычи звёздной валюты.", (120, 245, 160))
                ],
                badge_text="ФАРМ ЗВЁЗД",
                icons=[stellar_cactus_img_m]
            )
        else:
            draw_guide_card(
                "СБОРКА ДЛЯ БОГАТСТВА (ЭКОНОМИКА)",
                (255, 195, 75),
                [
                    ("• Что выставлять: ", "«Шахтёрский Жетон» (скидка на первые башни) + «Древний Кувшин» (доход ферм).", (240, 245, 255)),
                    ("• Назначение: ", "Для старта любого забега и легкого фарма на длинных картах.", (255, 215, 90)),
                    ("• Результат: ", "Позволяет моментально выставить фермы и мощную оборону в первые волны.", (140, 255, 180))
                ],
                badge_text="ФАРМ ЗОЛОТА",
                icons=[relic_icon, shovel_icon]
            )
            draw_guide_card(
                "БОЕВАЯ СБОРКА ПРОТИВ БОССОВ",
                (255, 140, 100),
                [
                    ("• Что выставлять: ", "«Око Бездны» (пробитие брони), «Громовой Зуб» (молнии) или реликвии стихий.", (240, 245, 255)),
                    ("• Назначение: ", "Для рекордных волн и рубежей, где идут толпы Титанов и плотные боссы.", (255, 180, 140)),
                    ("• Результат: ", "Максимальный взрывной урон и мгновенная ликвидация опасных врагов.", (140, 255, 180))
                ],
                badge_text="БОЙ И БОССЫ",
                icons=[relic_icon, tesla_tower_img]
            )
            draw_guide_card(
                ...
                (80, 220, 140),
                [
                    ("• Рецепт: ", "1 реликвия на экономику, 1 на замедление/контроль и остальные — под главный урон.", (240, 245, 255)),
                    ("• Гибкость: ", "Меняйте реликвии на пьедесталах в зависимости от биома и слабостей врагов.", (255, 215, 90)),
                    ("• Синергия: ", "Бонусы реликвий перемножаются с Древом талантов и Оранжереей.", (140, 255, 180))
                ],
                badge_text="БАЛАНС",
    ...
    # -------------------------------------------------------------
        ...
        if current_tab == 0:
            draw_guide_card(
                "КОМБО «ТЕРМОШОК» (ЛЁД + ОГОНЬ)",
                (255, 140, 40),
                [
                    ("• Суть связки: ", "Ледяная башня замедляет мобов, а Огненная наносит огромный комбо-урон по мерзлоте.", (240, 245, 255)),
                    ("• Слабости слаймов: ", "Ледяные слаймы уязвимы к огню, а Огненные — к ледяным атакам.", (255, 215, 80)),
                    ("• Совет: ", "Ставьте Лёд чуть раньше по дороге, а Огонь — сразу за ним на перекрёстках.", (130, 235, 160))
                ],
                badge_text="ТЕРМОШОК",
                icons=[rock_tower_img, freeze_tower_img]
            )
            draw_guide_card(
                "СВЯЗКА «ЖЕЛЕЗНЫЙ ЗАСЛОН» (ПАЛАТКА + СПЛЭШ)",
                (80, 225, 120),
                [
                    ("• Суть связки: ", "Воины из палатки останавливают мобов на дороге в плотную неподвижную кучу.", (240, 245, 255)),
                    ("• Массовый урон: ", "Магические башни и камнемёты разносят всю скопившуюся толпу за пару залпов.", (140, 255, 180)),
                    ("• Совет [R]: ", "Клавишей [R] выставляйте точку сбора воинов прямо в зону обстрела сплэш-башен.", (255, 215, 90))
                ],
                badge_text="ГАРНИЗОН",
                icons=[tent_tower_img, soldier_img]
            )
            draw_guide_card(
                "СВЯЗКА «ТУРБО-ОАЗИС» (ФЕРМА + СКОРОСТРЕЛЫ)",
                (255, 215, 60),
                [
                    ("• Аура Орошения: ", "Ферма регулярно поливает соседние башни, существенно ускоряя их стрельбу.", (240, 245, 255)),
                    ("• Пулемёт и Тесла: ", "Скорострельные башни и цепная молния под аурой полива зачищают волны в секунды.", (130, 240, 160)),
                    ("• Совет: ", "Стройте ферму в центре перекрёстка и окружайте её боевыми башнями со всех сторон.", (100, 235, 255))
                ],
                badge_text="СКОРОСТЬ И ДОХОД",
                icons=[farm_tower_img, cactus_img]
            )
        elif current_tab == 1:
            draw_guide_card(
                "ФИЗИЧЕСКИЙ УРОН И БРОНЯ МОБОВ",
                (80, 210, 255),
                [
                    ("• Кто наносит: ", "Обычные башни, снайперы, камнемёты и воины наносят физический урон.", (240, 245, 255)),
                    ("• Броня врагов: ", "Бронированные слаймы и Титаны срезают большую часть физического урона.", (255, 180, 180)),
                    ("• Совет: ", "Против тяжелой брони физ. башни малоэффективны — комбинируйте их с магией и Теслой.", (140, 255, 180))
                ],
                badge_text="ФИЗ. УРОН",
                icons=[magic_tower_img, tent_tower_img]
            )
            draw_guide_card(
                "ЧИСТЫЙ УРОН: МАГИЯ И ТЕСЛА",
                (90, 235, 255),
                [
                    ("• Пробитие брони: ", "Электричество Теслы и заклинания Магов на 100% игнорируют физическую броню!", (140, 255, 180)),
                    ("• Цепная молния: ", "Тесла мгновенно перескакивает по толпе слаймов, сжигая быстрых и бронированных врагов.", (240, 245, 255)),
                    ("• Совет: ", "Держите хотя бы 1–2 источника чистого урона к появлению тяжелых бронированных волн.", (255, 215, 80))
                ],
                badge_text="ЧИСТЫЙ УРОН",
                icons=[tesla_tower_img]
            )
            draw_guide_card(
                "ОБОРОНА БАЗЫ И ЗАЛП С ОРБИТЫ [F]",
                (210, 140, 255),
                [
                    ("• Запас жизней: ", "База выдерживает прорывы слаймов, а талант «Тёмный Эгис» поглощает их без урона.", (240, 245, 255)),
                    ("• Залп с орбиты [F]: ", "Стягивает врагов в черную дыру и мгновенно уничтожает упавший метеорит.", (255, 150, 210)),
                    ("• Фокус метеорита: ", "Клик по упавшему метеориту фокусирует огонь всех башен (+радиус и урон).", (255, 215, 100))
                ],
                badge_text="ОБОРОНА",
                icons=[heart_img, meteorite_img]
            )
        else:
            draw_guide_card(
                "СКЕЙЛИНГ ЦЕН: НЕ СПАМЬТЕ ОДНУ БАШНЮ!",
                (255, 215, 60),
                [
                    ("• Главное правило: ", "Каждая повторная покупка башни того же типа резко дорожает (вплоть до x4!).", (255, 215, 80)),
                    ("• Почему это важно: ", "Спамить одну и ту же башню крайне невыгодно — бюджет быстро иссякнет.", (255, 140, 120)),
                    ("• Совет: ", "Всегда стройте разнообразную армию из разных башен — это дешевле и сильнее.", (140, 245, 170))
                ],
                badge_text="НЕ СПАМИТЬ",
            ...
            draw_guide_card(
                ...
                (100, 220, 255),
                [
                    ("• Возврат средств: ", "Продажа возвращает 70% всех вложенных средств (покупка + все уровни апгрейда).", (140, 255, 180)),
                    ("• Сложность карт: ", "На дальних картах базовые цены башен немного выше из-за опасности биома.", (240, 245, 255)),
                    ("• Совет: ", "Не бойтесь продавать ранние слабые башни, чтобы перестроить оборону под боссов.", (255, 220, 120))
                ],
                badge_text="ВОЗВРАТ 70%",
                icons=[bounty_upg_icon, start_cacti_icon]
            )
            draw_guide_card(
                "ГОРЯЧИЕ КЛАВИШИ И ТЕМП",
                (255, 120, 150),
                [
                    ("• Скорость [Пробел]: ", "Переключает темп игры: 0.2X для точных решений, 1X–3X для боя, 5X для быстрого фарма.", (255, 215, 90)),
                    ("• Прокачка [МАКС]: ", "Кнопка [МАКС] улучшает выбранную башню сразу на все доступные деньги в 1 клик.", (255, 180, 200)),
                    ("• Меню и Пауза: ", "Клавиши [ESC] или [P] открывают меню паузы, бестиарий врагов и настройки звука.", (240, 245, 255))
                ],
