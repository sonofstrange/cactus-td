...
def draw_mechanics_guide_modal(surface, mouse_pos, current_tab=0, context="combat"):
    ...
    if context == "tree":
        ...
        sub_info_str = "Прокачка • Получение валюты • Метеориты и Орбиталка • Советы"
        tabs = [
            ("КАК РАБОТАЕТ ДРЕВО", (255, 195, 75)),
            ("ВАЛЮТА И МИДГЕЙМ", (195, 120, 255))
        ]
    elif context == "greenhouse":
        accent_col = (80, 220, 140)
        title_str = "СПРАВОЧНИК: ОРАНЖЕРЕЯ КАКТУСОВ"
        sub_info_str = "Пассивные бонусы • Ростки с боев • Прокачка сортов • Таланты древа"
        tabs = [
            ("КАК РАБОТАЕТ ОРАНЖЕРЕЯ", (80, 225, 130)),
            ("ПРОКАЧКА И ТАЛАНТЫ", (255, 210, 70))
        ]
    elif context == "relics":
        accent_col = (255, 180, 75)
        title_str = "СПРАВОЧНИК: МУЗЕЙ И РЕЛИКВИИ"
        sub_info_str = "Археология • Пьедесталы • Уровни реликвий 1..5 • Раскопки в бою"
        tabs = [
            ("МУЗЕЙ И ПЬЕДЕСТАЛЫ", (255, 195, 75)),
            ("РАСКОПКИ И ПРОКАЧКА", (185, 120, 255))
        ]
    else:
        accent_col = (65, 170, 240)
        title_str = "СПРАВОЧНИК: БОЙ, БАШНИ И ТАКТИКА"
        sub_info_str = "Связки башен • Типы урона • Скорость игры • Управление"
        tabs = [
            ("ЛУЧШИЕ СВЯЗКИ И КОМБО", (255, 160, 50)),
            ("ТИПЫ УРОНА И ОБОРОНА", (80, 220, 130)),
            ("УПРАВЛЕНИЕ И СКОРОСТЬ", (255, 215, 60))
        ]

    current_tab = current_tab % len(tabs)
    ...
    # 3. Вкладки (Tabs)
    tab_w = 360 if len(tabs) == 2 else 300
    tab_h = 36
    tab_gap = 12
    ...
    if context == "tree":
        if current_tab == 0:
            draw_guide_card(
                "ПРОКАЧКА И УЗЛЫ ДРЕВА",
                (255, 160, 50),
                [
                    ("• Постоянная сила: ", "Тратьте Звёздные кактусы на открытие узлов. Все бонусы действуют во всех боях.", (240, 245, 255)),
                    ("• Что дают узлы: ", "Повышают урон башен, стартовое золото, открывают новые типы башен и лимиты уровней.", (255, 215, 80)),
                    ("• Совет по старту: ", "В первую очередь качайте урон башен и стартовое золото — это ускорит фарм первых карт.", (140, 255, 180))
                ],
                badge_text="ПРОКАЧКА",
            ...
            draw_guide_card(
                ...
                (80, 210, 255),
                [
                    ("• Новые башни: ", "Открывает постройку Фермы, а позже — её усиление «Орошение» для ускорения соседних башен.", (240, 245, 255)),
                    ("• Лимиты уровней: ", "Повышает максимальный потолок прокачки башен в бою с базового 20 до 30 и выше.", (140, 255, 180)),
                    ("• Новые механики: ", "Открывает Археологию (раскопки), Оранжерею, Метеориты и Орбитальный Залп.", (255, 215, 90))
                ],
                badge_text="ВОЗМОЖНОСТИ",
                icons=[farm_tower_img, crown_upg_icon]
            )
            draw_guide_card(
                "СТРАТЕГИЯ РАЗВИТИЯ",
                (100, 235, 150),
                [
                    ("• Экономика: ", "Ранний прирост золота даёт возможность сразу выставлять башни с 1-й волны.", (240, 245, 255)),
                    ("• Урон и база: ", "Если слаймы прорываются — повышайте урон башен и здоровье кактуса-базы.", (255, 215, 80)),
                    ("• Без спешки: ", "В игре нет сброса древа, но звёзды добываются бесконечно — прокачать можно всё!", (140, 255, 180))
                ],
                badge_text="СОВЕТЫ",
                icons=[bounty_upg_icon, damage_upg_icon]
            )
        else:
            draw_guide_card(
                "ЗВЁЗДНЫЕ КАКТУСЫ (ОСНОВНАЯ ВАЛЮТА)",
                (255, 205, 80),
                [
                    ("• Источники: ", "Выпадают со слаймов, гарантированно с боссов (каждые 5 волн) и в раскопках.", (240, 245, 255)),
                    ("• Рубежи волн: ", "Прохождение 10, 25 и 50 волн даёт солидные награды Звёздными кактусами.", (255, 215, 80)),
                    ("• Повторный фарм: ", "Повторное прохождение открытых биомов приносит стабильный приток звёзд.", (140, 255, 180))
                ],
                badge_text="ДОБЫЧА ЗВЁЗД",
                icons=[stellar_cactus_img_m]
            )
            draw_guide_card(
                "МЕТЕОРИТЫ И ТЁМНЫЙ РЕСУРС (МИДГЕЙМ)",
                (195, 120, 255),
                [
                    ("• Требует прокачки: ", "Метеориты появляются только после изучения узла «Астральный Маяк» в древе.", (255, 215, 80)),
                    ("• Тёмные кактусы: ", "Упавший метеорит нужно разбить башнями — из него выпадают Тёмные кактусы.", (220, 160, 255)),
                    ("• Назначение: ", "Тёмная валюта расходуется на самые мощные поздние таланты Древа.", (140, 255, 180))
                ],
                badge_text="МИДГЕЙМ",
                icons=[dark_cactus_img, meteorite_img]
            )
            draw_guide_card(
                "ОРБИТАЛЬНЫЙ ЗАЛП [F] (МИДГЕЙМ)",
                (120, 220, 255),
                [
                    ("• Требует прокачки: ", "Орбиталка [F] открывается в древе за тёмные кактусы — на старте её нет!", (255, 180, 140)),
                    ("• Эффект воронки: ", "Затягивает врагов в чёрную дыру, нанося урон, и мгновенно раскалывает метеорит.", (140, 255, 180)),
                    ("• Применение: ", "Идеально активировать против опасных толп врагов и для быстрого сбора метеоритов.", (240, 245, 255))
                ],
                badge_text="КЛАВИША [F]",
    ...
    # -------------------------------------------------------------
        ...
        if current_tab == 0:
            draw_guide_card(
                "ПОСТОЯННЫЕ БОНУСЫ ВО ВСЕХ БОЯХ",
                (80, 225, 130),
                [
                    ("• Суть механики: ", "Каждый выращенный кактус даёт постоянный пассивный бонус во всех боях на всех картах.", (240, 245, 255)),
                    ("• 8 уникальных сортов: ", "Каждый сорт усиливает свою сферу: здоровье базы, доход, урон башен, молнии и т.д.", (140, 255, 180)),
                    ("• Без рутины: ", "Никаких леек и таймеров — бонусы работают постоянно и усиливают весь ваш профиль.", (255, 215, 90))
                ],
                badge_text="ПАССИВКА НАВСЕГДА",
            ...
            draw_guide_card(
                ...
                (140, 240, 160),
                [
                    ("• Источники: ", "Ростки кактусов выпадают во время боя с поверженных волн и гарантированно с боссов.", (240, 245, 255)),
                    ("• Накопление: ", "Ростки автоматически суммируются. Чем больше волн и боссов пройдено, тем больше запас.", (140, 255, 180)),
                    ("• Боссы каждые 5 волн: ", "Победа над каждым боссом приносит наибольшее количество ростков.", (255, 215, 90))
                ],
                badge_text="ДОБЫЧА РОСТКОВ",
                icons=[sprout_icon]
            )
            draw_guide_card(
                "ПОВЫШЕНИЕ УРОВНЯ СОРТА",
                (255, 210, 70),
                [
                    ("• Повышение уровня: ", "Когда набралось необходимое число ростков, нажмите кнопку улучшения кактуса.", (240, 245, 255)),
                    ("• Рост характеристик: ", "С каждым уровнем пассивный эффект кактуса становится заметно мощнее.", (140, 255, 180)),
                    ("• Совет: ", "Качайте все сорта параллельно для всестороннего усиления вашей базы и башен.", (255, 215, 80))
                ],
                badge_text="ПРОКАЧКА СОРТОВ",
                icons=[crown_upg_icon, sprout_icon]
            )
        else:
            draw_guide_card(
                ...
                (80, 210, 255),
                [
                    ("• База и золото: ", "Сагуаро повышает HP базы и шипы, Опунция даёт бонус к стартовому золоту.", (240, 245, 255)),
                    ("• Воины и стихии: ", "Мамиллярия усиливает воинов Палатки, Огненный и Ледяной сорта растят стихийный урон.", (140, 255, 180)),
                    ("• Молния и бездна: ", "Эхинокактус усиливает Теслу, а Астрофитум пробивает защиту тяжелых слаймов.", (220, 160, 255))
                ],
                badge_text="8 СОРТОВ",
                icons=[gh_tex.get("gh_saguaro", cactus_img), gh_tex.get("gh_fire_barrel", cactus_img)]
            )
            draw_guide_card(
                "ТАЛАНТ «РЕЗОНАНС ФЛОРЫ»",
                (255, 195, 75),
                [
                    ("• Суть таланта: ", "Увеличивает силу ВСЕХ бонусов оранжереи на +10% за каждый уровень узла в Древе.", (240, 245, 255)),
                    ("• Глобальный охват: ", "Одновременно усиливает здоровье базы, стартовое золото, урон огня, льда и молний.", (140, 255, 180)),
                    ("• Совет: ", "Открывайте в Древе талантов, когда у вас уже подросли базовые уровни кактусов.", (255, 215, 90))
                ],
            ...
            draw_guide_card(
                ...
                (100, 230, 255),
                [
                    ("• Суть таланта: ", "Даёт бонус к урону ВСЕХ башен (+0.2% за каждый суммарный уровень сортов).", (255, 220, 100)),
                    ("• Общий вклад: ", "Каждый уровень любого, даже самого простого кактуса делает всю армию башен сильнее!", (140, 255, 180)),
                    ("• Совет: ", "Не забрасывайте дешевые начальные сорта — они дают быстрый суммарный уровень.", (240, 245, 255))
                ],
    ...
    # -------------------------------------------------------------
        ...
        if current_tab == 0:
            draw_guide_card(
                "ПЬЕДЕСТАЛЫ (АКТИВНЫЕ РЕЛИКВИИ)",
                (255, 210, 60),
                [
                    ("• Как работает: ", "Выставляйте найденные реликвии на пьедесталы музея, чтобы активировать эффекты в бою.", (240, 245, 255)),
                    ("• Число слотов: ", "Базово доступно 2 пьедестала. Прокачка узлов в Древе расширяет их число до 5 слотов.", (140, 255, 180)),
            ...
            draw_guide_card(
                "УРОВНИ РЕЛИКВИЙ (БАЗОВЫЙ 1, В ДРЕВЕ ДО 5)",
                (255, 215, 75),
                [
                    ("• Начальный лимит: ", "По умолчанию максимальный уровень реликвии равен 1.", (240, 245, 255)),
                    ("• Прокачка капа: ", "Изучение узла в Древе улучшений повышает потолок прокачки артефактов до 5 уровня!", (255, 215, 90)),
                    ("• Рост бонусов: ", "Повторные раскопки повышают уровень реликвии, пропорционально умножая её бонус.", (140, 255, 180))
                ],
                badge_text="МАКС. 1..5 УР.",
            ...
            draw_guide_card(
                ...
                (195, 120, 255),
                [
                    ("• Пассивная мощь: ", "Позволяет всей коллекции музея давать пассивный бонус даже без пьедесталов!", (220, 160, 255)),
                    ("• Сила коллекции: ", "Каждая найденная и прокачанная реликвия продолжает работать на оборону вашей базы.", (140, 255, 180)),
                    ("• Совет: ", "Открывайте этот эндгейм-талант в Древе, чтобы раскрыть весь потенциал собранного музея.", (255, 220, 100))
                ],
                badge_text="ПАССИВКА",
            ...
            draw_guide_card(
                "РАСКОПКИ В РЕАЛЬНОМ ВРЕМЕНИ [R]",
                (255, 195, 75),
                [
                    ("• Появление кургана: ", "Во время волны на карте возникает древний курган. Клик или [R] открывает раскопки.", (240, 245, 255)),
                    ("• Без паузы: ", "Раскопки НЕ ставят игру на паузу — бой на карте продолжается в реальном времени!", (255, 120, 120)),
                    ("• Тактика: ", "Перед раскопками переключайте скорость на 0.2X, чтобы враги не успели пробить базу.", (140, 255, 180))
                ],
                badge_text="БЕЗ ПАУЗЫ!",
                icons=[shovel_icon]
            )
            draw_guide_card(
                "АРТЕФАКТЫ ПО БИОМАМ",
                (100, 220, 255),
                [
                    ("• Распределение: ", "Каждая карта скрывает ровно по 2 уникальные тематические реликвии под свой биом.", (240, 245, 255)),
                    ("• Сетка 5х5: ", "В сетке раскопок скрыты фигуры артефактов (1х2, 1х3, 2х2). Находите клетки с деталями.", (140, 255, 180)),
                    ("• Разнообразие: ", "Экономика, урон стихий, радиус башен, цепные молнии — ищите их на разных картах.", (220, 160, 255))
                ],
                badge_text="2 НА БИОМ",
                icons=[relic_icon]
            )
            draw_guide_card(
                "УСЛОВИЕ ПОЯВЛЕНИЯ РАСКОПОК",
                (80, 240, 150),
                [
                    ("• Когда исчезают: ", "Курганы раскопок ПЕРЕСТАЮТ появляться на карте, если обе её реликвии замакшены.", (255, 215, 90)),
                    ("• Смена карты: ", "Если на текущей карте раскопок больше нет — переходите на другие биомы.", (140, 255, 180)),
                    ("• Прокачка капа: ", "Увеличивайте лимит уровня реликвий в древе, чтобы снова копать их до 5 уровня.", (240, 245, 255))
                ],
                badge_text="ЛИМИТ КАРТЫ",
                icons=[trophy_icon, shovel_icon]
            )
    ...
    # -------------------------------------------------------------
        ...
        if current_tab == 0:
            draw_guide_card(
                ...
                (255, 140, 40),
                [
                    ("• Суть связки: ", "Ледяная башня замедляет мобов, а Огненная наносит огромный сплэш-урон по площади.", (240, 245, 255)),
                    ("• Бонус по мерзлоте: ", "Огненная башня наносит существенно увеличенный урон по замороженным врагам!", (255, 215, 80)),
                    ("• Совет: ", "Ставьте Лёд перед скоплением, а Огонь — на перекрёстках для максимального сплэша.", (130, 235, 160))
                ],
                badge_text="СПЛЭШ + СЛОУ",
                icons=[rock_tower_img, freeze_tower_img]
            )
            draw_guide_card(
                "СВЯЗКА «ЖЕЛЕЗНЫЙ ЗАСЛОН» (ПАЛАТКА + СПЛЭШ/ЦЕПЬ)",
                (80, 225, 120),
                [
                    ("• Суть связки: ", "Воины из Палатки задерживают слаймов на дороге, собирая их в плотную толпу.", (240, 245, 255)),
                    ("• Массовый урон: ", "Огненная башня (сплэш) и Тесла (цепная молния) быстро выжигают скопившихся врагов.", (140, 255, 180)),
                    ("• Совет [R]: ", "Клавишей [R] задавайте точку сбора воинов прямо в радиусе действия Огня и Теслы.", (255, 215, 90))
                ],
                badge_text="КОНТРОЛЬ ДОРОГИ",
                icons=[tent_tower_img, soldier_img]
            )
            draw_guide_card(
                "СВЯЗКА «ТУРБО-ОАЗИС» (ФЕРМА + ОРОШЕНИЕ)",
                (255, 215, 60),
                [
                    ("• Доход и бафф: ", "Ферма даёт золото, а после изучения узла «Орошение» в древе — ускоряет соседей!", (240, 245, 255)),
                    ("• Требует прокачки: ", "Полив и ускорение атаки башен работают ТОЛЬКО при изученном таланте в Древе.", (255, 180, 100)),
                    ("• Совет: ", "Окружайте прокачанную ферму боевыми башнями, чтобы получить максимальный темп стрельбы.", (140, 245, 170))
                ],
                badge_text="РАЗГОН АТАК",
                icons=[farm_tower_img, cactus_img]
            )
        elif current_tab == 1:
            draw_guide_card(
                "МАГИЯ И ТЕСЛА: ОДИНОЧНЫЙ И ЦЕПНОЙ УРОН",
                (80, 210, 255),
                [
                    ("• Магическая башня: ", "Огромный одиночный урон и высокий шанс критического удара. БЕЗ сплэша!", (240, 245, 255)),
                    ("• Башня Тесла: ", "Выпускает цепную молнию, поражающую сразу группу врагов серией рикошетов.", (140, 255, 180)),
                    ("• Совет: ", "Магия уничтожает жирных боссов, а Тесла идеально выкашивает стаи быстрых слаймов.", (255, 215, 80))
                ],
                badge_text="МАГИЯ И ЦЕПЬ",
                icons=[magic_tower_img, tesla_tower_img]
            )
            draw_guide_card(
                "ОГНЕННАЯ БАШНЯ: ЕДИНСТВЕННЫЙ СПЛЭШ",
                (255, 140, 80),
                [
                    ("• Урон по площади: ", "Огненная башня стреляет снарядами с взрывным сплэш-уроном по площади.", (240, 245, 255)),
                    ("• Эффект горения: ", "Поджигает врагов и наносит дополнительный периодический урон огнём.", (255, 180, 100)),
                    ("• Совет: ", "Ставьте на изгибах дорог, где слаймы идут плотными рядами.", (140, 255, 180))
                ],
                badge_text="СПЛЭШ ПО ПЛОЩАДИ",
                icons=[rock_tower_img]
            )
            draw_guide_card(
                "ОРБИТАЛКА [F] И МЕТЕОРИТЫ (МИДГЕЙМ)",
                (210, 140, 255),
                [
                    ("• Требуют древа: ", "Падение метеоритов и Орбитальный Залп [F] требуют предварительной прокачки в древе.", (255, 215, 90)),
                    ("• Фокус метеорита: ", "Клик по метеориту фокусирует огонь всех башен для быстрой добычи тёмных кактусов.", (140, 255, 180)),
                    ("• Орбиталка [F]: ", "Стягивает мобов в воронку и раскалывает упавший метеорит одним ударом.", (240, 245, 255))
                ],
                badge_text="ТРЕБУЕТ ДРЕВА",
                icons=[meteorite_img, dark_cactus_img]
            )
        else:
            draw_guide_card(
                "ТАКТИКА СКОРОСТИ ИГРЫ",
                (255, 215, 60),
                [
                    ("• Режим 0.2X: ", "Всегда переключайтесь на 0.2X для постройки, улучшений и продумывания тактики.", (140, 255, 180)),
                    ("• Максимальная скорость: ", "Для битвы включайте максимум: базовая 3X, а при прокачке в древе — до 10X!", (255, 215, 80)),
                    ("• Быстрый бой: ", "Высокая скорость экономит кучу времени при фарме уже освоенных волн.", (240, 245, 255))
                ],
                badge_text="СКОРОСТЬ ИГРЫ",
                icons=[crown_upg_icon, speed_upg_icon]
            )
            draw_guide_card(
                "УПРАВЛЕНИЕ СКОРОСТЬЮ",
                (80, 210, 255),
                [
                    ("• Клик ЛКМ по кнопке: ", "Увеличивает скорость игры (0.2X → 0.5X → 1X → 2X → 3X ... до 10X).", (240, 245, 255)),
                    ("• Клик ПКМ по кнопке: ", "Понижает скорость игры обратно шаг за шагом.", (255, 215, 80)),
                    ("• Клавиша [Пробел]: ", "Быстро переключает скорость игры между обычной и ускоренной.", (140, 255, 180))
                ],
                badge_text="ЛКМ / ПКМ",
                icons=[speed_upg_icon]
            )
            draw_guide_card(
                "СКЕЙЛИНГ ЦЕН И УЛУЧШЕНИЯ",
                (255, 120, 150),
                [
                    ("• Не спамьте один тип: ", "Каждая следующая купленная башня того же типа становится дороже!", (255, 150, 150)),
                    ("• Баланс башен: ", "Стройте разные типы башен — это существенно дешевле и даёт нужные синергии.", (140, 255, 180)),
                    ("• Кнопка [МАКС]: ", "Улучшает выбранную башню сразу на все доступные деньги за один клик.", (240, 245, 255))
                ],
